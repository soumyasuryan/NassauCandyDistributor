...
def get_supabase_client():
    """
    Get Supabase client. Returns None if Supabase is not available or configured.
    """
    ...
    try:
        ...
        # Silently fail if Supabase connection fails
        return None
...
def save_run_to_supabase(run_name, df_clean):
    """
    Save analysis run to Supabase. Returns run_id if successful, None if Supabase unavailable.
    """
    ...
    if client is None:
        # Supabase not available, skip saving
        return None
    ...
    try:
        ...
        # Silently fail if save operation fails
        return None


def get_saved_runs():
    """
    Get saved analysis runs from Supabase. Returns empty DataFrame if unavailable.
    """
    client = get_supabase_client()
    if client is None:
        return pd.DataFrame()
    ...
    try:
        response = client.table("analysis_runs").select("*").order("created_at", desc=True).execute()
...
def load_run_data(run_id):
    """
    Load run data from Supabase. Returns empty DataFrame if unavailable.
    """
    ...
    try:
        response = client.table("run_orders").select("*").eq("run_id", run_id).execute()
